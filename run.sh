...
log "Executing initial flag generator."
if [[ -x "${PROJECT_ROOT}/data/get_flag.sh" ]]; then
    FLAG_OUTPUT="$("${PROJECT_ROOT}/data/get_flag.sh")"
    log "data/get_flag.sh produced: ${FLAG_OUTPUT}"
else
    abort "data/get_flag.sh is missing or not executable."
fi

...
- :class:`TimeoutGuard` is a context manager for begrudging the code a narrow
  time window. It uses :mod:`signal` when available, else falls back to a
  watchdog thread that is annoyed into action.
...
class TimeoutGuard:
    """
    Context manager that enforces a wall clock timeout. The class prefers using
    :func:`signal.setitimer` because it is delightfully abrupt. If signals are
    missing (e.g. Windows, unwilling kernels, or philosophical runtime
    environments), it falls back to a watchdog thread that yells at the running
    code by raising :class:`JailTimeout`.
    ...
    _thread: Optional[threading.Timer] = dataclasses.field(init=False, default=None)
    _old_handler: Optional[signal.Handlers] = dataclasses.field(init=False, default=None)
    ...
    def _handle_signal(self, signum, frame) -> None:  # pragma: no cover - signal path
        self._expired = True
        raise JailTimeout(f"time budget exceeded for {self.label!r}")
    ...
    def __enter__(self) -> "TimeoutGuard":
        ...
        else:  # pragma: no cover - fallback used on non-Unix systems
            self._thread = threading.Timer(self.seconds, self._trigger_thread_timeout)
            self._thread.daemon = True
            self._thread.start()

        return self

    def _trigger_thread_timeout(self) -> None:  # pragma: no cover - fallback path
        self._expired = True
        raise JailTimeout(f"time budget exceeded for {self.label!r}")
...
class PythonJail:
    ...
    def _ensure_paths(self) -> None:
        ...
        self.fake_flag_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def drop_fake_flag(self) -> None:
        """
        Deploy the fake flag for curious adventurers. We overwrite the file
    ...
    def _normalise_vm_payload(self, payload, encoding: str = "latin-1") -> bytes:
        ...
        if isinstance(payload, str):
            ...
            except LookupError as exc:  # pragma: no cover - depends on user input
        ...
        if not data:
            raise ValueError("vm_escape payload must not be empty")

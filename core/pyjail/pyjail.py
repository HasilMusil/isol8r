...
except AttributeError:  # pragma: no cover - PyPy / limited builds
    _PY_SET_ASYNC_EXC = None
else:  # pragma: no cover - CPython fast path
...
class TimeoutGuard:
    ...
    def _timeout_triggered(self) -> None:  # pragma: no cover - timer thread
    ...
    def _raise_in_target_thread(self, exc_type: Type[JailTimeout]) -> None:
        ...
        if thread_id is None:
            raise exc_type()
        if _PY_SET_ASYNC_EXC is None:  # pragma: no cover - PyPy / limited builds
            raise exc_type()
